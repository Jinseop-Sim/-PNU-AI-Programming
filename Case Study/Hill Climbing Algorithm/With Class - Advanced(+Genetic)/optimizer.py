...
class Genetic(MetaHeruistics):
    ...
    def setVariables(self, parameters):
        MetaHeruistics.setVariables(self, parameters)
    ...
    def displaySetting(self):
        ...
        print()
        MetaHeruistics.displaySetting(self);
        print()
    ...
    def run(self, p):
        ...
        ch = 0
        while ch < self._limitEval:
            newPop = []
            fitSum = 0
            for i in range(0, len(population)):
                p.evalInd(population[i])
                fitSum = fitSum + population[i][0]
            population.sort(reverse = True)

            for i in range(0, len(population)):
                x, y = self.randomSelection(population, fitSum)
                child_x, child_y = p.crossover(x, y, self._uXp)
                child = p.mutation(child_x, self._mrF)
                newPop.append(child)
            population = newPop
            ch += 1

        value = p.indToSol(population[0])
        bestValue = p.evaluate(value)
        p.storeResult(value, bestValue)

    def randomSelection(self, population, fitSum):
        prob = []
        probIdx = []

        for i in range(len(population)):
            probIdx.append(i)
            prob.append(population[i][0]/fitSum)

        x, y = rn.choice(probIdx, 2, p=prob, replace=False)
